...
m.configureInfoPlist = function(app_path, config, callback) {
    ...
    fs.writeFile(app_path + '/Contents/Info.plist', plist.build(properties), {
        encoding: 'utf8'
    }, function(error) {
        callback(error);
    });
...
m.configureHelpers = function(app_path, bundle_id, callback) {
    ...
    for (var index = 0; index < files.length; index += 1) {
        try {
            var plist = fs.readFileSync(files[index], {
                encoding: 'utf8'
            });
            fs.writeFileSync(files[index], plist.replace('io.nwjs.nwjs', bundle_id), {
                encoding: 'utf8'
            });
        } catch (error) {
            callback(error);
